...
```{r}
forwardStepwise <- function(dataset){
  if(is.data.frame(dataset)){
    ...
    plot(elem.summary$rsq, xlab="model size", ylab="R-squared", type = "l")
    # plot showing BIC on the y-axis and
    plot(summary$bic, xlab="Model Size", ylab="BIC",type = "l")
    return(summary)
  }
}

elem.stepWise <- forwardStepwise(elementary.reg)
#print(which.min(elem.stepWise$bic))
#coef(elementary.reg, which.min(elem.stepWise$bic))
middle.stepWise <- forwardStepwise(middle.reg)
high.stepWise <- forwardStepwise(high.reg)
```
...
#full <- data.frame(elem.x,elem.y)
set.seed(1)
new.elem.train <- sample(1:nrow(elementary.reg), nrow(elementary.reg)/2)

#elem.glm <- glmnet(elem.x[elem.train,],elem.y[elem.train],family = "binomial",alpha=1) 
#elem.glm

new <- as.data.frame(elementary.reg[new.elem.train,])
elementary.tree <- rpart(API~., data = new, method = "class")

#plot
plot(elementary.tree)
text(elementary.tree, pretty = 0)
print(elementary.tree)
