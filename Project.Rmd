...
9. **Random Forest** : We then ran random forests on the training data and then tested it on our test data. We came out with about 80% accuracy. We also had nice varimp plots that we found this very useful. 
...
** All the models pick AVG_ED as the most important. If you see on our Random Forests, this is the most important for all three school levels. ALso in our pruned decisions trees(based on minimum BIC): elementary and middle school only had AVG_ED, while highschool had AVG_ED and VALID). Most models also fit VALID, WH_SIG, AS_SIG, YRS_TEACH, NUMTEACH, and AS_SIG. There is some variation between the school levels which we will discuss in the next question **
