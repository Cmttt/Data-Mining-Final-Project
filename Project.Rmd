...
#Random Forest - Elementary School
```{r}
...
#Random Forest - Middle School
```{r}
middle.train=middle.train %>% mutate_if(is.character, as.factor)
middle.rf <- randomForest(API~., data = middle.train, mtry=6, importance=TRUE)
print(middle.rf)
importance(middle.rf)
varImpPlot(middle.rf)
```

#Random Forest - High School
```{r}
high.train=high.train %>% mutate_if(is.character, as.factor)
high.rf <- randomForest(API~., data = high.train, mtry=6, importance=TRUE)
print(high.rf)
importance(high.rf)
varImpPlot(high.rf)
```
